...
public class RaftSpecificationTests
{
    ...
    public void ElectionSafety_AtMostOneLeaderPerTerm()
    {
        ...
        foreach (var kvp in leadersByTerm)
        {
            ...
            Assert.That(leaders.Count, Is.LessThanOrEqualTo(1),
        ...
        Assert.That(totalLeaders, Is.EqualTo(1),
    ...
    public void ElectionSafety_ServerVotesOncePerTerm()
    {
        ...
        Assert.That(voter.VotedFor, Is.EqualTo(candidate1.ServerId), "Should remember who was voted for");
    }
    ...
    public void LeaderAppendOnly_NeverOverwritesOrDeletesEntries()
    {
        ...
        log.Add(new LogEntry { Term = 1, Index = 1, Command = "cmd1" });
        log.Add(new LogEntry { Term = 1, Index = 2, Command = "cmd2" });
        log.Add(new LogEntry { Term = 2, Index = 3, Command = "cmd3" });
        ...
        Assert.That(log.Count, Is.GreaterThanOrEqualTo(originalLogCount), 
            "Leader log should never shrink");

        // Verify original entries are unchanged
        for (int i = 0; i < originalLogCount; i++)
        {
            Assert.That(log[i].Term, Is.EqualTo(originalEntries[i].Term), 
                $"Entry {i} term should not change");
            Assert.That(log[i].Command, Is.EqualTo(originalEntries[i].Command), 
                $"Entry {i} command should not change");
            Assert.That(log[i].Index, Is.EqualTo(originalEntries[i].Index), 
    ...
    public void LogMatching_IdenticalEntriesImplyIdenticalPrefixes()
    {
        ...
        log1.Add(new LogEntry { Term = 1, Index = 1, Command = "cmd1" });
        log1.Add(new LogEntry { Term = 1, Index = 2, Command = "cmd2" });
        log1.Add(new LogEntry { Term = 2, Index = 3, Command = "cmd3" });

        log2.Add(new LogEntry { Term = 1, Index = 1, Command = "cmd1" });
        log2.Add(new LogEntry { Term = 1, Index = 2, Command = "cmd2" });
        log2.Add(new LogEntry { Term = 2, Index = 3, Command = "cmd3" });

        // Add different entries after the matching point
        log1.Add(new LogEntry { Term = 2, Index = 4, Command = "cmd4a" });
        log2.Add(new LogEntry { Term = 3, Index = 4, Command = "cmd4b" });
        ...
        if (log1[matchingIndex - 1].Term == matchingTerm && log2[matchingIndex - 1].Term == matchingTerm)
        {
            for (int i = 0; i < matchingIndex; i++)
            {
                Assert.That(log1[i].Term, Is.EqualTo(log2[i].Term), 
                    $"Log matching violated: entries at index {i + 1} have different terms");
                Assert.That(log1[i].Command, Is.EqualTo(log2[i].Command), 
    ...
    public void LogMatching_AppendEntriesRejectsInconsistentLogs()
    {
        ...
        log.Add(new LogEntry { Term = 1, Index = 1, Command = "cmd1" });
        log.Add(new LogEntry { Term = 2, Index = 2, Command = "cmd2" });
        ...
        var args = new AppendEntriesArgs
        {
            ...
            {
                new() { Term = 3, Index = 3, Command = "cmd3" }
            },
            LeaderCommit = 0
        };
    ...
    public void LeaderCompleteness_NewLeaderHasAllCommittedEntries()
    {
        ...
        oldLog.Add(new LogEntry { Term = 1, Index = 1, Command = "committed_cmd" });
        ...
        newLog.Add(new LogEntry { Term = 1, Index = 1, Command = "committed_cmd" });
        ...
        var newLeaderHasEntry = newLog.Any(e => e.Index == committedEntry?.Index && 
                                                e.Term == committedEntry.Term && 
                                                e.Command == committedEntry.Command);

        Assert.That(newLeaderHasEntry, Is.True, 
    ...
    public void LeaderCompleteness_VoteRejectedForIncompleteLog()
    {
        ...
        log.Add(new LogEntry { Term = 2, Index = 1, Command = "cmd1" });
        log.Add(new LogEntry { Term = 3, Index = 2, Command = "cmd2" });
    ...
    public void StateMachineSafety_SameIndexSameEntry()
    {
        ...
        var entry = new LogEntry { Term = 1, Index = 1, Command = "same_cmd" };
        ...
        Assert.That(server1.Log.Count, Is.GreaterThanOrEqualTo(1), "Server1 should have at least one log entry");
        Assert.That(server2.Log.Count, Is.GreaterThanOrEqualTo(1), "Server2 should have at least one log entry");

        var logEntry1 = server1.Log[0];
        var logEntry2 = server2.Log[0];

        Assert.That(logEntry1.Index, Is.EqualTo(logEntry2.Index), "Log entries should have same index");
        Assert.That(logEntry1.Term, Is.EqualTo(logEntry2.Term), "Log entries should have same term");
        Assert.That(logEntry1.Command, Is.EqualTo(logEntry2.Command), "Log entries should have same command");
    ...
    public void ElectionTimeout_TriggersStateTransition()
    {
        ...
        Assert.That(server.CurrentTerm, Is.GreaterThan(0), "Term should increment when becoming candidate");
    ...
    public void HeartbeatResetsElectionTimeout()
    {
        ...
        Assert.That(newTimeout, Is.Not.EqualTo(originalTimeout),
            "Election timeout should be reset after receiving heartbeat");
        
        // Also verify it's set to a future time
        Assert.That(newTimeout, Is.GreaterThan(DateTime.UtcNow.AddMilliseconds(-1000)),
    ...
    public void TermComparison_UpdatesStateCorrectly()
    {
        ...
        Assert.That(server.VotedFor, Is.EqualTo(string.Empty).Or.EqualTo("candidate"), 
